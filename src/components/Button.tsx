...
export function Button({ variant = 'primary', className, ...props }: ButtonProps) {
  return (
    <button
      className={clsx(
        'inline-flex items-center justify-center px-token-4 py-token-2 text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
        ...
        variant === 'primary' && 'bg-brand-primary text-white hover:opacity-90 rounded-token-lg',
        variant === 'secondary' &&
        ...
        variant === 'gradient' && 'bg-button-gradient text-white hover:opacity-90 rounded-token-md',
        className,
      )}
      {...props}
    />
